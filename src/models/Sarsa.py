...
def SARSA(env, episodes = 1000000, alpha = 0.1, gamma = 0.6, epsilon = 0.1):
    ...
    for i in range(1, episodes + 1):
        ...
        while not done:
            ...
            action = random_action(action, epsilon, number_actions)
            ...
            if next_state[0] < 21:
                ...
                next_action = np.argmax(next_to_consider) # Exploit learned values
                next_action = random_action(next_action, epsilon, number_actions)
                next_value = q_table[next_state_code][next_action]

                # correct_new_value considering the formula in the slides
                # correct_new_value = old_value + alpha * (reward + (gamma * next_value) - old_value)
